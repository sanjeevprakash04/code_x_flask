...
def getFTSE_TAG_FRQ(df):
        # Create a textfile and write text in it - file is later exported to csv
        file = open("export/rockwell/ftseTags/RA_FTSE_TAGS_FRQ.csv", "w+", encoding="cp1252")
        ...
        for i in range(len(df)):
            tag_name_raw = f'{df.loc[i, "Tag"]}'
            cabinet_raw = f'{df.loc[i, "Tag Part 2"]}'
            tag_comment_raw = f'{df.loc[i, "Description"]}'
            tag_name = tag_name_raw.replace(".1", "").replace(".2", "").replace(".3", "").replace(".4", "").replace(".5", "")
            cabinet = cabinet_raw.replace(".1", "").replace(".2", "").replace(".3", "").replace(".4", "").replace(".5", "")
            tag_comment = tag_comment_raw.replace(",", ".") # Commas are not allowed in .csv files when comma is set as seperator
            tag_adress = f'{cabinet} - Eth. IP: {df.loc[i, "IP Address"]}'
        ...
        folder_df.to_csv('export/rockwell/ftseTags/RA_FTSE_TAGS_FRQ.csv', mode='a', header=False, index=False, sep=',', quoting=csv.QUOTE_NONE, encoding="cp1252")
        file = open("export/rockwell/ftseTags/RA_FTSE_TAGS_FRQ.csv", "a+")
        file.write("\n;Tag Section\n")
        file.close()
        tag_df.to_csv('export/rockwell/ftseTags/RA_FTSE_TAGS_FRQ.csv', mode='a', header=False, index=False, sep=',', quoting=csv.QUOTE_NONE, encoding="cp1252")
