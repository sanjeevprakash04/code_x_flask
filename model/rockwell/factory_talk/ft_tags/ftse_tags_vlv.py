...
def getFTSE_TAG_VLV(df):
        ...
        for i in range(len(df)):
            ...
            tag_adress = f'{df.loc[i, "IO-address"]}'
            activateCaption = df.loc[i, "Activate Text"] if pd.notna(df.loc[i, "Activate Text"]) else "na"
            deactivateCaption = df.loc[i, "Deactivate Text"] if pd.notna(df.loc[i, "Deactivate Text"]) else "na"
            negativeFB = df.loc[i, "Valve FB N"] if pd.notna(df.loc[i, "Valve FB N"]) else "na"
            positiveFB = df.loc[i, "Valve FB P"] if pd.notna(df.loc[i, "Valve FB P"]) else "na"
            
            area = f'{df.loc[i, "Area"]}'
            actCap_dict = {'Tag Type': '"S"',
                    'Tag Name': f'"{tag_name}_{identifier}\ActivateCaption"',
                    'Tag Description' : '"Energize button caption"',
                    'Read Only' : '"F"',
                    'Data Source' : '"M"',
                    'Security Code' : '"*"',
                    'Alarmed' : '"F"',
                    ...
                    'Initial Digital' : '',
                    'Length String' : '82',
                    'Initial String' : f'"{activateCaption}"',
                    'Retentive' : '0',
                    'Address' : ''
                    }
            dactCap_dict = {'Tag Type': '"S"',
                    'Tag Name': f'"{tag_name}_{identifier}\DeactivateCaption"',
                    'Tag Description' : '"Deenergize button caption"',
                    'Read Only' : '"F"',
                    'Data Source' : '"M"',
                    'Security Code' : '"*"',
                    'Alarmed' : '"F"',
                    ...
                    'Initial Digital' : '',
                    'Length String' : '82',
                    'Initial String' : f'"{deactivateCaption}"',
                    'Retentive' : '0',
                    'Address' : ''
                    }    
            eu_dict = {'Tag Type': '"S"',
            ...
            negFb_dict = {'Tag Type': '"S"',
                    'Tag Name': f'"{tag_name}_{identifier}\\Negative_FB"',
                    'Tag Description' : '"Negative feedback"',
                    'Read Only' : '"F"',
                    'Data Source' : '"M"',
                    'Security Code' : '"*"',
                    'Alarmed' : '"F"',
            ...
            posFb_dict = {'Tag Type': '"S"',
                    'Tag Name': f'"{tag_name}_{identifier}\Positive_FB"',
                    'Tag Description' : '"Positive feedback"',
                    'Read Only' : '"F"',
                    'Data Source' : '"M"',
                    'Security Code' : '"*"',
                    'Alarmed' : '"F"',
            ...
            tag_list.append(actCap_dict)
            tag_list.append(dactCap_dict)
            tag_list.append(eu_dict)
            tag_list.append(negFb_dict)
            tag_list.append(plc_dict)
            tag_list.append(posFb_dict)
