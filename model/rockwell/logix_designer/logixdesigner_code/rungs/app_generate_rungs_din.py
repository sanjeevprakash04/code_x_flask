...
def getDinRungs(df, nr, Cmd_AlmDint):
    ...
    Routine = etree.SubElement(Routines, 'Routine', Use="Context", Name="R030_DIN")
    ...
    for i in range(len(df)):
        if (str(df.loc[i, "PLC No."]) == str(nr)):
            ...
            tag_din_udt = f'{df.loc[i, "Tag"]}_DIN_UDT'
            tag_cm_din = f'{df.loc[i, "Tag"]}_DIN_CM'
            ...
            if Cmd_AlmDint:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_DIN({tag_cm_din},PLANT,{var_sectionLocal},DisplayNames,{tag_plc_name},{tag_din_udt},{var_tabId},{var_subId},{var_almDint});')
            else:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_DIN({tag_cm_din},PLANT,{var_sectionLocal},DisplayNames,{tag_plc_name},{tag_din_udt},{var_tabId},{var_subId});')


    tree = etree.ElementTree(RSLogix5000Content)
    etree.indent(tree, space="")
    tree.write('export/rockwell/rungs/RA_RUNGS_DIN.l5x', pretty_print=True, encoding="utf-8", xml_declaration=True)