...
def getAoutRungs(df, nr, Cmd_AlmDint):
    ...
    Routine = etree.SubElement(Routines, 'Routine', Use="Context", Name="R038_AOUT")
    ...
    for i in range(len(df)):
        if (str(df.loc[i, "PLC No."]) == str(nr)):
            ...
            tag_aout_udt = f'{df.loc[i, "Tag"]}_AOUT_UDT'
            tag_cm_aout = f'{df.loc[i, "Tag"]}_AOUT_CM'
            ...
            if Cmd_AlmDint:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_AOUT({tag_cm_aout},PLANT,{var_sectionLocal},DisplayNames,{tag_aout_udt},{var_tabId},{var_subId},{tag_plc_name},{var_almDint});')
            else:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_AOUT({tag_cm_aout},PLANT,{var_sectionLocal},DisplayNames,{tag_aout_udt},{var_tabId},{var_subId},{tag_plc_name});')

    tree = etree.ElementTree(RSLogix5000Content)
    etree.indent(tree, space="")
    tree.write('export/rockwell/rungs/RA_RUNGS_AOUT.l5x', pretty_print=True, encoding="utf-8", xml_declaration=True)