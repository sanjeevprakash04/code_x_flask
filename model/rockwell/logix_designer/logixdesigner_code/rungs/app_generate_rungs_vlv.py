...
def getVlvRungs(df, nr, Cmd_AlmDint):
    ...
    Routine = etree.SubElement(Routines, 'Routine', Use="Context", Name="R050_VLV")
    ...
    for i in range(len(df)):
        if (str(df.loc[i, "PLC No."]) == str(nr)):
            ...
            tag_vlv_udt = f'{no_dot_name}_VLV_UDT'
            tag_cm_vlv = f'{no_dot_name}_VLV_CM'
            tag_plc_name = f'{df.loc[i, "Valve Out P"]}'
            tag_l_safe = f'L_Safe_{df.loc[i, "Tag Part 3"]}'
            if pd.notna(df.loc[i, "Valve FB P"]): tag_inp_fb_p = f'{df.loc[i, "Valve FB P"]}'
            else: tag_inp_fb_p = "Off"
            if pd.notna(df.loc[i, "Valve FB N"]): tag_inp_fb_n = f'{df.loc[i, "Valve FB N"]}'
            else: tag_inp_fb_n = "Off"
            if pd.notna(df.loc[i, "Valve Out P"]): tag_out_p = f'{df.loc[i, "Valve Out P"]}'
            else: tag_out_p = "Dummy_Out_P"
            if pd.notna(df.loc[i, "Valve Out N"]): tag_out_n = f'{df.loc[i, "Valve Out N"]}'
            else: tag_out_n = "Dummy_Out_N"
            ...
            if Cmd_AlmDint:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_VLV({tag_cm_vlv},PLANT,{var_sectionLocal},DisplayNames,Off,{tag_inp_fb_p},{tag_inp_fb_n},{tag_vlv_udt},{var_tabId},{var_subId},{tag_out_p},{tag_out_n},{var_almDint});')
            else:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_VLV({tag_cm_vlv},PLANT,{var_sectionLocal},DisplayNames,Off,{tag_inp_fb_p},{tag_inp_fb_n},{tag_vlv_udt},{var_tabId},{var_subId},{tag_out_p},{tag_out_n});')

    tree = etree.ElementTree(RSLogix5000Content)
    etree.indent(tree, space="")
    tree.write('export/rockwell/rungs/RA_RUNGS_VLV.l5x', pretty_print=True, encoding="utf-8", xml_declaration=True)