...
def getMesRungs(df, nr, Cmd_AlmDint):
    var_rungNr = 52
    var_rungCount = 0
    # Get total number of rungs to create
    for x in range(len(df)):
        if f'{df.loc[x, "Description"]}' == "Mes" and not pd.isna(df.loc[x, "Area"]) and (str(df.loc[x, "PLC No."]) == str(nr)):
    ...
    Routine = etree.SubElement(Routines, 'Routine', Use="Context", Name="R019_MES")
    RLLContent = etree.SubElement(Routine, 'RLLContent', Use="Context")

    # Import of rungs for alarms
    for i in range(len(df)):
        if f'{df.loc[i, "Tag Part 2"]}' == "Mes" and (str(df.loc[i, "PLC No."]) == str(nr)):        
            var_panelName = f'{df.iloc[i, 7]}'
            var_comment = f'{df.iloc[i, 4]}'
            tag_alm_udt = f'{df.loc[i, "Tag"]}_ALM_UDT'
            tag_cm_alm = f'{df.loc[i, "Tag"]}_ALM_CM'
            ...
            if Cmd_AlmDint:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_ALM({tag_cm_alm},PLANT,{var_sectionLocal},DisplayNames,{tag_plc_name},{tag_alm_udt},{var_tabId},{var_subId},{var_almDint});')
            else:
                etree.SubElement(Rung, 'Text').text = etree.CDATA(f'CM_ALM({tag_cm_alm},PLANT,{var_sectionLocal},DisplayNames,{tag_plc_name},{tag_alm_udt},{var_tabId},{var_subId});')

    tree = etree.ElementTree(RSLogix5000Content)
    etree.indent(tree, space="")
    tree.write('export/rockwell/rungs/RA_RUNGS_MES.l5x', pretty_print=True, encoding="utf-8", xml_declaration=True)