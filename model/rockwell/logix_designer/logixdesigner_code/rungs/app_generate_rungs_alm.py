...
def getAlmRungs(df, nr, Cmd_AlmDint):
    ...
    for x in range(len(df)):
        if f'{df.loc[x, "Description"]}' != "Mes" and (str(df.loc[x, "PLC No."]) == str(nr)):
    ...
    Routine = etree.SubElement(Routines, 'Routine', Use="Context", Name="R020_ALM")
    ...
    for i in range(len(df)):
        if (str(df.loc[i, "PLC No."]) == str(nr)):
            ...
            # Generates rungs for 'ALM' alarms - (NOT FOR MES ALARMS)
    ...
    tree.write('export/rockwell/rungs/RA_RUNGS_ALM.l5x', pretty_print=True, encoding="utf-8", xml_declaration=True)